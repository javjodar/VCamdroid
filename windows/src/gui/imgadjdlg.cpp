...
ImgAdjDlg::ImgAdjDlg(wxWindow* parent, const DeviceDescriptor& deviceDesc)
	: wxDialog(parent, wxID_ANY, "Image Adjustments", wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
	...
	{
		// Skip the "None" category (usually just contains "No Filter")
		...
		if (category == Video::Filter::Category::CORRECTION)
		{
			...
			{
				// Create a slider for every filter in this list
				AddSlider(adjustmentsSizer->GetStaticBox(), adjustmentsGrid, filterName, 50);
				hasSliders = true;
			}
		}
		...
		else
		{
			...
			choices.Add("None"); // Always add a "Turn Off" option
			...
			choice->SetSelection(0); // Select "None" initially

			// Bind Event
			// We use the category ID to identify this dropdown later
...
void ImgAdjDlg::OnReset(wxCommandEvent& event)
{
	...
	for (auto& [name, slider] : sliders) {
		...
		// We trigger the event so the app updates the live stream
		wxCommandEvent evt(EVT_FILTER_PARAM_CHANGED);
		evt.SetString(name);
		evt.SetInt(50);
		ProcessWindowEvent(evt);
